...
test.describe('Mobile DeviantArt Core Membership Tests - iPhone 14 Pro Max', () => {

  test('Mobile Core Membership comprehensive flow per requirements', async ({ browser }) => {
    console.log('📱 Starting comprehensive mobile core membership test...');
    
    const expectedTitles = ['Core Pro+ Member', 'Core+ Member', 'Core Pro Member', 'Core Max Member'];
    
    // Create iPhone 14 Pro Max context
    ...
    logSuccess('Mobile page loaded');
    
    await page.screenshot({ 
      path: `screenshots/mobile-core-membership-main-${timestamp}.png`,
    ...
    if (!billingModalExists) {
      await page.screenshot({ 
        path: `screenshots/mobile-billing-modal-debug-${timestamp}.png`, 
        clip: { x: 0, y: 0, width: 430, height: 800 }
      });
    ...
    await page.screenshot({ 
      path: `screenshots/mobile-billing-cycle-modal-${timestamp}.png`, 
      clip: { x: 0, y: 0, width: 430, height: 800 }
    });
    
    logSuccess('Comprehensive mobile test completed!');
    await context.close();
  });
}); 